-- CORRECTED FIX: All ambiguous column references with proper type casting
-- This fixes the text = uuid error

-- Drop the broken function completely
DROP FUNCTION IF EXISTS update_order_status(UUID, TEXT, TIMESTAMPTZ);

-- Create the CORRECTED fixed version with ALL column qualifiers
...
DECLARE
    ...
    v_update_data JSONB;
BEGIN
    -- Get current status - QUALIFY ALL COLUMN REFERENCES with explicit casting
    SELECT orders.status INTO v_old_status 
    FROM orders 
...
-- CORRECTED TEST with proper type casting
DO $$
DECLARE
    test_result RECORD;
    ...
    found_order_id UUID;
BEGIN
    RAISE NOTICE '🧪 Testing CORRECTED update_order_status function with order: %', test_order_id;
    
    -- First, verify this order exists and get its actual ID
    SELECT id INTO found_order_id FROM orders WHERE id = test_order_id;
    
    IF found_order_id IS NULL THEN
        RAISE NOTICE '⚠️ Test order not found, trying to find any paid order...';
        
        -- Try to find any paid order for testing
        ...
        IF found_order_id IS NULL THEN
            RAISE EXCEPTION 'No paid orders found for testing';
        ELSE
            RAISE NOTICE '✅ Using existing paid order for testing: %', found_order_id;
        END IF;
    ELSE
        RAISE NOTICE '✅ Test order found: %', found_order_id;
    END IF;
    
    -- Show current status before update
    SELECT orders.status INTO test_result FROM orders WHERE orders.id = found_order_id;
    RAISE NOTICE '📊 Before update: status = %', test_result.status;
    
    BEGIN
        -- Call the function with explicit UUID casting
        ...
        FROM orders 
        WHERE orders.id = found_order_id;
