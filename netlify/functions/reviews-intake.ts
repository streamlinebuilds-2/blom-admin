...
export const handler: Handler = async (e) => {
  ...
  try {
    const { product_id, name, rating, title, body, images } = JSON.parse(e.body || "{}");
    ...
    if (process.env.REVIEWS_INTAKE_WEBHOOK) {
      await fetch(process.env.REVIEWS_INTAKE_WEBHOOK, {
        ...
        body: JSON.stringify({ review_id: data.id, product_id, name, rating, title, body, images, status: "pending" })
      }).catch(()=>{});
    }
