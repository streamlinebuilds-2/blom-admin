...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const base = process.env.URL || process.env.SITE_URL || process.env.PUBLIC_SITE_URL || 'https://blom-cosmetics.co.za';
    try {
      const ac = new AbortController();
      const t = setTimeout(() => ac.abort(), 12000);
      try {
        await fetch(`${base.replace(/\/$/, '')}/.netlify/functions/invoice-pdf?return_url=1`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ m_payment_id: orderId, return_url: true }),
          signal: ac.signal
        });
      } finally {
        clearTimeout(t);
      ...
      console.error('Invoice generation (ITN):', e);
    }
