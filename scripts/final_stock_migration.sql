-- Final Stock Management System Migration Script
-- This script handles all edge cases including existing functions
...
-- Drop existing function if it exists, then recreate
DROP FUNCTION IF EXISTS log_stock_movement(uuid, integer, text, uuid, integer, text, text);
...
-- Drop existing function if it exists, then recreate with different name to avoid conflicts
...
SELECT 'Final Stock Management System migration completed successfully!' as status;